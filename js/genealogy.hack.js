$(document).ready(function () {
  ...
  function traverseDeepFirst_FindPersonChildren(personId, callback) {
...
//          $(this).parents('.tree-level-container').nextAll('.tree-level-container').remove();
        ...
        traverseDeepFirst_FindPersonChildren(nodePersonId, function(childrenArray) {
          ...
          var fragment = _childrenFragment( childrenArray , treeLevel , parent );

          $context.parents('.boxOut-person').parent().parent().append( fragment );

          // width adjustment
          var tempWidth = $context.parents('.normalizeTreeNode').next().width();
          var paddingWidth = 30;
          var additionalWidth = tempWidth - boxWidth;
          var containerWidth = $('#treeContainer').width();

          if(childrenArray.length > 1) paddingWidth *= childrenArray.length;
          else additionalWidth = 0;

          var newWidth = containerWidth + additionalWidth + paddingWidth;
//alert( additionalWidth + paddingWidth);          
          $('#treeContainer').width((newWidth)+"px");  
        ...
        var paddingWidth = 30;
        var additionalWidth = $(this).parents('.boxOut-person').parent().parent().width();
        var containerWidth = $('#treeContainer').width();
